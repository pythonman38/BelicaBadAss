...
void AEnemy::SetComponentOverlaps()
{
...
void AEnemy::SetCollisionResponses()
{
	// Block bullets so they cause damage
...
void AEnemy::AgroSphereBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		...
		if (Character && EnemyController && EnemyController->GetBlackboardComponent()) EnemyController->GetBlackboardComponent()->SetValueAsObject(TEXT("Target"), Character);
	}
}
...
void AEnemy::CombatRangeBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		...
		{
			bInAttackRange = true;
			if (EnemyController) EnemyController->GetBlackboardComponent()->SetValueAsBool(TEXT("InAttackRange"), bInAttackRange);
		}
	}
}
...
void AEnemy::PlayAttackMontage(FName Section, float PlayRate)
{
	...
	if (EnemyController) EnemyController->GetBlackboardComponent()->SetValueAsBool(FName("CanAttack"), bCanAttack);
}
...
void AEnemy::DoDamage(AShooterCharacter* Victim)
{
	if (Victim)
	{
		...
		if (Victim->GetMeleeImpactSound()) UGameplayStatics::PlaySoundAtLocation(this, Victim->GetMeleeImpactSound(), GetActorLocation());
	}
}
...
void AEnemy::SpawnBlood(AShooterCharacter* Victim, FName SocketName)
{
	...
	if (TipSocket)
	{
		...
		if (Victim->GetBloodParticles()) UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), Victim->GetBloodParticles(), SocketTransform);
	}
}
...
void AEnemy::StunCharacter(AShooterCharacter* Victim)
{
	if (Victim)
	{
		...
		if (Stun <= Victim->GetStunChance()) Victim->Stun();
	}
}
